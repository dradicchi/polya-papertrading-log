...
The paper-trading system publishes four parallel instances: the **combined short-volatility book** — now run as a single genuinely merged live book (`full_combined_daily`) — alongside its two component legs (short-CALL and short-PUT, also run standalone) and a no-override **reference benchmark**. **Each instance publishes its own reports** under `instances/<id>/reports/` (`weekly/` and `monthly/`); each report leads with the **cumulative-since-go-live** metrics, then the **cycle window**:

| Instance | Status | Reports |
|---|---|---|
| `full_combined_daily` | Active since Aug 10 | **primary — merged short-CALL + short-PUT book**, daily-only, both legs traded under one stream (independent legs: each opens when it has an eligible candidate) — [weekly](instances/full_combined_daily/reports/weekly/) · [monthly](instances/full_combined_daily/reports/monthly/) |
...
| `full_overrides` | Active since May 9 | short-**CALL** leg run standalone (3 horizons) — [weekly](instances/full_overrides/reports/weekly/) · [monthly](instances/full_overrides/reports/monthly/) |
| `put_full_overrides` | Active since Jul 13 | short-**PUT** leg run standalone, daily-only (near-ATM mechanical selection) — [weekly](instances/put_full_overrides/reports/weekly/) · [monthly](instances/put_full_overrides/reports/monthly/) |

_Primary strategy of interest: the merged **`full_combined_daily`** book (short CALL + short PUT, daily), evaluated against the `canonical` reference. The standalone `full_overrides` (CALL) and `put_full_overrides` (PUT) legs are published so the combined book can be decomposed. `full_combined_daily` is a single primary event stream (its own signed, timestamped ledger), not an analytical overlay._
...
- `instances/` holds the four active streams that make up the current track record: `full_combined_daily` (the primary merged short-CALL + short-PUT book, daily-only, both legs under one stream), `canonical` (reference benchmark), and the standalone `full_overrides` (CALL, 3 horizons) + `put_full_overrides` (PUT, daily) legs — published so the merged book can be decomposed. `full_combined_daily` is a genuine single ledger, not an analytical overlay; standalone-leg overlays, when shown separately, are labeled as such (disclaimer #3 still holds: all-option, no perpetual hedge). Five earlier streams — `legacy_v2.1`, `partial_overrides`, `partial_clean`, `partial_k2_2pct`, `partial_k2_5pct` — were **frozen on 2026-07-30** and are retained under `instances/<id>/` for audit continuity only (each carries a `RETIRED.md`); they are not part of the active track record.
